...
function CorpSetupForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleInit = async () => {
    if (!account || !corpName.trim()) return;
    setBusy(true); setErr(null);
    try {
      const tx = buildInitializeCorpTransaction(corpName.trim(), account.address);
      const signer = new CurrentAccountSigner(dAppKit);
      const result = await signer.signAndExecuteTransaction({ transaction: tx });
      // Extract created object IDs — Treasury is the third shared object created
      const ids = extractCreatedIds(result);
      if (ids.length >= 3) {
        // ids[0]=Registry, ids[1]=Corp, ids[2]=Treasury (in creation order)
        // We can't reliably know which is which without type info, so cache all
        // and let the next fetch sort it out by querying MemberCap → corpId
        console.log("[CradleOS] created objects:", ids);
        // Store last ID as treasury hint; user can correct via connect
  ...
  return (
    <div className="card" style={{ maxWidth: "440px" }}>
      ...
      <button
        ...
        onClick={handleInit}
        disabled={busy || !corpName.trim() || !account}
